...
public class AttendanceExcepDao extends BaseDao implements IAttendanceExcepDao {
	...
	public int insertNormalRecords(Map<String, Object> saveMap) {
		...
		if (RunUtil.isNotEmpty(saveMap.get("exists"))) {
			updateSql.append(" UPDATE CLOCKREOCRDS SET AMSB = CASE WHEN  AMSB IS NOT NULL OR TO_DATE(TO_CHAR(SYSDATE, 'YYYY-MM-DD') || ' ' || AMSB, 'YYYY-MM-DD HH24:Mi:SS') > ");
			updateSql.append(" (SELECT TO_DATE(TO_CHAR(SYSDATE, 'YYYY-MM-DD') || ' ' || AMSBTIME,'YYYY-MM-DD HH24:Mi:SS') FROM CLOCK_SETTING WHERE ISENABLE = '0')");
			updateSql.append(" THEN (SELECT  AMSBTIME  FROM CLOCK_SETTING WHERE ISENABLE = '0') ELSE AMSB END,");
			
			updateSql.append("  AMXB = CASE WHEN AMXB IS NOT NULL OR TO_DATE(TO_CHAR(SYSDATE, 'YYYY-MM-DD') || ' ' || AMXB, 'YYYY-MM-DD HH24:Mi:SS') < ");
			updateSql.append(" (SELECT TO_DATE(TO_CHAR(SYSDATE,'YYYY-MM-DD') || ' ' || AMXBTIME,'YYYY-MM-DD HH24:Mi:SS')  FROM CLOCK_SETTING  WHERE ISENABLE = '0')");
			updateSql.append(" THEN (SELECT AMXBTIME   FROM CLOCK_SETTING  WHERE ISENABLE = '0') ELSE AMXB END,");
			

			updateSql.append("  PMSB = CASE WHEN  PMSB IS NOT NULL OR TO_DATE(TO_CHAR(SYSDATE, 'YYYY-MM-DD') || ' ' || PMSB, 'YYYY-MM-DD HH24:Mi:SS') > ");
			updateSql.append(" (SELECT TO_DATE(TO_CHAR(SYSDATE, 'YYYY-MM-DD') || ' ' || PMSBTIME,'YYYY-MM-DD HH24:Mi:SS') FROM CLOCK_SETTING  WHERE ISENABLE = '0') ");
			updateSql.append(" THEN (SELECT PMSBTIME FROM CLOCK_SETTING  WHERE ISENABLE = '0') ELSE PMSB END,");
			

			updateSql.append("  PMXB = CASE WHEN  PMXB IS NOT NULL OR TO_DATE(TO_CHAR(SYSDATE, 'YYYY-MM-DD') || ' ' || PMXB, 'YYYY-MM-DD HH24:Mi:SS') < ");
			updateSql.append(" (SELECT TO_DATE(TO_CHAR(SYSDATE, 'YYYY-MM-DD') || ' ' || PMXBTIME,'YYYY-MM-DD HH24:Mi:SS') FROM CLOCK_SETTING WHERE ISENABLE = '0')");
			updateSql.append(" THEN (SELECT PMXBTIME FROM CLOCK_SETTING WHERE ISENABLE = '0') ELSE PMXB END");
