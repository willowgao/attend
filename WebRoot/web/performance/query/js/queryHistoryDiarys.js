 //jQuery将$换成jq，避免冲突
	...
	function getData() { 
		...
		jq.ajax( {
			url : programName + '/query/queryDiarys!queryHistoryDiarys.action',
			type : 'post',
			data : params,
			dataType : 'json',
    ...
    var query = function(){
	    ...
	    jq('#dg').datagrid({
	        url: programName + '/query/queryDiarys!queryHistoryDiarys.action?starttime=' + starttime + '&endtime=' + endtime+ '&diarytype=' + diarytype 
	    });
    }
